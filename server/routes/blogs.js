...
router.get("/featured", (req, res) => {
  db.Blog.findAll({
    ...
    blogs ? res.status(200).json(blogs) : res.status(404).send("404 not found");
  });
});
...
router.get("/:id", (req, res) => {
  ...
  db.Blog.findById(blogId).then(blog => {
    blog ? res.status(200).json(blog) : res.status(404).send("404 no blog");
  });
});
...
router.put("/:id", (req, res) => {
  ...
  db.Blog.update(req.body, {
    ...
    blog ? res.status(204).json(blog) : res.status(404).send("404 not updated");
  });
});
...
router.delete("/:id", (req, res) => {
  ...
  db.Blog.destroy({
    ...
    blog ? res.status(200).send("Blog deleted") : res.status(404).send("404 not deleted");
  });
});
